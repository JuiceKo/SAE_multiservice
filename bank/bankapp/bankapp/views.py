...
async def depot(request):
    nc = await nats.connect("10.128.200.7")
    if request.method == 'POST':
        ...
        await nc.publish("deposit", str(montant).encode())
        print(montant)
        print("Deposit request published successfully.")
        await nc.close()
        ...
        async def alerte(request):
            nc = await nats.connect("10.128.200.7")
            if request.method == 'POST':
                iban = request.POST.get('iban')  # Récupérer l'IBAN à partir des données POST
                mounmoun = float(request.POST.get('montant'))  # Récupérer le montant à partir des données POST
                print(montant)
                if mounmoun > 10000:
                    notification = f"Le montant de la transaction est de {mounmoun} et excède $10,000"
                    await nc.publish("notification", notification.encode())
                else:
                    # Si le montant du retrait est <= $10,000
                    pass
                    await nc.close()
                    if __name__ == '__main__':
                        asyncio.run(main())
...
async def retrait(request):
    nc = await nats.connect("10.128.200.7")
    if request.method == 'POST':
        iban = request.POST.get('iban')  # Récupérer l'IBAN à partir des données POST
        montant = float(request.POST.get('montant'))  # Récupérer le montant à partir des données POST
        await nc.publish("withdrawal", str(montant).encode())
        print(montant)
        print("Withdrawal request published successfully.")
        await nc.close()
        if __name__ == '__main__':
            asyncio.run(main())
        ...
        async def alerte(request):
            ...
            if request.method == 'POST':
                ...
                print(montant)
                if montant > 10000:
                    notification = f"Le montant de la transaction est de {montant} et excède $10,000"
                    await nc.publish("notification", notification.encode())
                else:
                    # Si le montant du retrait est <= $10,000
                    pass
